...
#	if defined(PR_SET_MM) && defined (PR_SET_MM_ENV_START) && defined (PR_SET_MM_ENV_END)
		...
		for (unsigned i(0); i < 2U; ++i) {
			bool success(true);
			errno = 0;
			if (0 > prctl(PR_SET_MM, PR_SET_MM_ENV_START, env_holder.data(), 0, 0)) {
				if (errno == EPERM) return;
				success = false;
			}
			errno = 0;
			if (0 > prctl(PR_SET_MM, PR_SET_MM_ENV_END, env_holder.data(), 0, 0)) {
				if (errno == EPERM) return;
				success = false;
			}
			if (success) break;
		...
		for (std::string::iterator p(env_holder.begin()), e(env_holder.end()); e != p; ++p)
			*p = '\0';
		env_holder = e;
		for (unsigned i(0); i < 2U; ++i) {
			bool success(true);
			...
			errno = 0;
			if (0 > prctl(PR_SET_MM, PR_SET_MM_ENV_END, env_holder.data() + env_holder.size(), 0, 0)) {
				if (errno == EPERM) return;
				success = false;
			}
			if (success) break;
